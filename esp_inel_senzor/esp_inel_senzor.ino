...
#define PIN_SENZOR2 23                  //senzorul de la finalul plasei care transmite dacă „bariera” dintre emițător și receptor a fost tăiată de minge
#define PIN_SENZOR3 18                  //senzorul de la baza inelului care transmite dacă „bariera” dintre emițător și receptor a fost tăiată de minge
...
{  
  Com_Bluetooth.begin("ESP_Inel1_Dispozitiv"); //inițializarea comunicării prin Bluetooth, iar dispozitivul va fi găsit cu numele de ”ESP_Inel2_Dispozitiv”
...
void loop()                                     //metodă ce se execută, în buclă, permanent
{
  ...
  stare_senzor3_inel = digitalRead(PIN_SENZOR3);    //citirea senzorului 2 de la baza inelului
  while (Com_Bluetooth.available())                 //cât timp există date ce sunt transmise prin Bluetooth către acest ESP, acestea vor fi interpretate
  {
    ...
    {  
      ...
      if(comandă[0]=='3')                           //dacă comanda începe cu cifra 3, înseamnă că mesajele ce vin, se ocupă de gestionarea timpului de joc și a scorului
      {
        ...
        if (sir.equals("A"))                        //dacă mesajul este A înseamnă că arbitrul a apăsat butonul de anulare al ultimului coș marcat,
        {                                           //dacă mesajul nu este însoțit și de a, atunci înseamnă că arbitrul a anulat ultimul coș pe perioada unei pauze, deci nu se mai poate marca nimic
          comandă_curentă = 'A';                    
        }
        if (sir.equals("a"))                        //dacă mesajul este a, înseamnă că arbitrul a decis anularea ultimei aruncări, 
        ...
        {                                           //la pauza mare, panourile se schimbă, iar pentru a păstra corectitudinea pe tabela de marcaj, trebuie inițializat scorul după pauză
          ...
          String val = sir.substring(start+1);      //scorul ce va fi după pauza mare, este scorul echipei B, deoarece codul rulează pentru panoul echipei A, inițial
      ...
      if(comandă[0]=='4')                           //dacă mesajul începe cu 4, înseamnă ca vine de la cameră și ne transmite distanța de la minge la cele 2 inele
      {
        ...
        int indexA = d.indexOf('A');                //extragem poziția literei A din mesaj
        int indexB = d.indexOf('B');                //extragem poziția literei B din mesaj
        String dist = d.substring(indexA+1, indexB);//fiind considerat primul inel, ne interesează distanța ce se află între A și B
  ...
  if((stare_senzor1_inel == 0 || stare_senzor3_inel==0) && înscriere_permisă)   //dacă mingea a tăiat partea superioară de inel și conform mesajelor, este permisă înscrierea punctelor
  {                                                                             //verificăm ce valoare are coșul marcat
    ...
    while(timp_curent - timp_anterior < 1000)                                    //se iau măsurători timp de 1 s de la senzorul de jos pt a vedea dacă mingea a trecut pe acolo
    {
      stare_senzor2_inel = digitalRead(PIN_SENZOR2);                            //dacă se consideră o aruncare validă, adică senzorul de jos s-a activat, urmează determinarea nr de puncte a coșului
      if(stare_senzor2_inel == 0)
      { 
        if(comandă_curentă == 'L' ||  comandă_curentă == 'a')                   //suntem în situația când sunt permise doar aruncările libere
        {
 ...
 if(comandă_curentă =='A' && (scor != scor_stergere))                           //suntem în cazul în care arbitrul a decis anularea ultimului coș 
