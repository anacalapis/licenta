...
adresa_ESP1 = "D4:8A:FC:A4:64:5A"                    #adresa MAC a ESP-ului ce se ocupă de procesarea datelor de la modulul ce se ocupă de pornirea/oprirea timpului și ștergerea ultimului coș
...
def conectare_la_ESP(addr):                                 #funcția ce se ocupă de conectarea prin Bluetooth la un ESP32, ce este dat prin adresa MAC ca și argument
    ...
    for s in servicii:                                       #cautam in lista de servicii oferita de acel dispozitiv
        if "1101" in s["service-classes"]:                   #servicul de tip Serial Port Profile (SPP) pt a putea stabili o conexiune "seriala" virtuala
            sock = BluetoothSocket(RFCOMM)                   #crează un socket pe protocolul RFCOMM
            sock.connect((s["host"], s["port"]))             #se conectează la adresa și portul dispozitivului ESP32
            print(f"Conectat la {addr}")                     #se afișează un mesaj de succes
            return sock                                      #se returnează socket-ul

    print(f"Nu s-a gasit un serviciu SPP")                   #se afișează un mesaj de eroare
    return None
