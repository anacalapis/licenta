...
porturi_seriale_posibile=['/dev/ttyACM0', '/dev/ttyACM1']
ser= None
for port in porturi_seriale_posibile:
...
def imu_camera(sock_imu, sock_inelA, sock_inelB, identifier):
    ...
    while True:
        if (datetime.now() - start_timp).seconds <1:
            ...
            if data_imu:
                linii_imu= data_imu.splitlines()
                for linie in linii_imu:
                    vector_imu[i] = linie
            ...
            index=0
            for x in range(i):
                if int(vector_imu[x].strip()) > int(val_max):
