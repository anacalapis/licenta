...
colt_stanga_sus_X, colt_stanga_sus_Y, lung_chenar, latime_chenar = 0, 0, 0, 0
obiect_selectat = False
limita_inferioara_culoare = None
limita_superioara_culoare = None
...
def scrie_distanta_in_fisier(X, Y):                     #funcție care este responsabilă de scrierea într-un fișier al unui mesaj 
    ...
    with lock:                                          #pentru a nu permite altui fir de execuție să efectueze operații asupra fișierului
        with open("distanta.txt", "w") as file:         #fișierul este deschis pentru scriere și se va face referire la acesta prin variabila ”file”
            ...
            distanta_inel_A = math.sqrt((X + 68.5)**2 + (Y +6)**2)                                          #coordonata inelului A (-68.5,-6)
            distanta_inel_B = math.sqrt((X - 70)**2 + (Y -9.5)**2)                                          #coordonata inelului B (70, 9.5)
          
            file.write(f'A{round(distanta_inel_A,3)}B{round(distanta_inel_B,3)}')                           #scriere în fișier după șablonul dorit
            distanta_inel_A = f"{round(distanta_inel_A,3)}"                                                 #am transformat distanțele în șiruri de caractere pentru a putea fi printate pe ecran
            distanta_inel_B = f"{round(distanta_inel_B,3)}"                         
            
            cv2.putText(clona, distanta_inel_A, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)   #pentru a știi distanțele ce sunt scrise în fișier
            cv2.putText(clona, distanta_inel_B, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)   #le-am printat și pe ecran
...
def desenare_chenar_culoare(event, x, y, flags, param):                          #funcție specifică OpenCV care monitorizează evenimentele mouse-ului
    ...
    global colt_stanga_sus_X, colt_stanga_sus_Y                                  #coordonatele colțului din stânga sus
    global lung_chenar, latime_chenar                                            #lățimea și lungimea dreptunghiului
    global limita_inferioara_culoare, limita_superioara_culoare                  #limita inferioară și superioară al intervalului de valori a culorii selectate în HSV
    if event == cv2.EVENT_LBUTTONDOWN:                                           #când este apăsat butonul stâng al mouse-ului, înseamnă că începe selectarea obiectului
        ...
        if conturare_obiect:                                                     #indică începutul selecții
            colt_stanga_sus_X, colt_stanga_sus_Y = min(coord_init_click_X, x), min(coord_init_click_Y, y)      #se actualizează în permanență coordonatele colțului din stânga-sus
            lung_chenar, latime_chenar = abs(coord_init_click_X - x), abs(coord_init_click_Y - y)    #se actualizează în permanență lungimea și lățimea dreptunghiului ce urmează să se formeze
        ...
        roi = cadru_HSV[colt_stanga_sus_Y:colt_stanga_sus_Y+latime_chenar, colt_stanga_sus_X:colt_stanga_sus_X+lung_chenar]      #se extrage zona selectată, în format HSV  
        ...
        limita_superioara_culoare = np.array([
        ...
        print(f"Intervalul de culoare [ {limita_inferioara_culoare} , {limita_superioara_culoare}]")    #se afișează, în terminal, intervalul obținut

def desenare_axe(imagine_curenta, centru_img_X, centru_img_Y):
    ...
    cv2.line(imagine_curenta, (int(centru_img_X), int(centru_img_Y)), (int(centru_img_X + 50), int(centru_img_Y)), (0, 0, 255), 2)
    ...
    cv2.putText(imagine_curenta, "X", (int(centru_img_X + 55), int(centru_img_Y + 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    cv2.line(imagine_curenta, (int(centru_img_X), int(centru_img_Y)), (int(centru_img_X), int(centru_img_Y - 50)), (0, 255, 0), 2)
    ...
    cv2.putText(imagine_curenta, "Y", (int(centru_img_X - 15), int(centru_img_Y - 55)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    cv2.circle(imagine_curenta, (int(centru_img_X), int(centru_img_Y)), 5, (255, 0, 0), -1)
    ...
    cv2.putText(imagine_curenta, "Z", (int(centru_img_X + 15), int(centru_img_Y + 25)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
...
camera_RGB = pipeline.createColorCamera()                       #adaugă o cameră RGB în pipeline, cu care se vor captura imagini color
camera_RGB.setPreviewSize(lungime_img, latime_img)              #se specifică dimensiunile (lungimea și lățimea) imaginii pe care camera o furnizează
camera_RGB.setInterleaved(False)                                #se specifică ca fiecare canal de culoare (R, G, B) să fie stocat separat pentru o procesare mai ușoară
camera_RGB.setFps(30)                                           #se specifică rata de imagini pe secundă (frames per second)

camera_stereo = pipeline.create(dai.node.StereoDepth)                              #se adaugă o cameră stereo folosită, în final, pentru calcularea distanței
camera_stereo.setDefaultProfilePreset(dai.node.StereoDepth.PresetMode.DEFAULT)     #se setează parametrii camerei în modul implicit
camera_stereo.setLeftRightCheck(True)                                              #este o filtrare a punctelor false, deoarece acesta verifică că obiectele sunt detectate de ambele camere
camera_stereo.setExtendedDisparity(False)                                          #este setat pentru detecția cu o precizie mai bună a obiectelor mai departe de 75 cm față de cameră
camera_stereo.setSubpixel(True)                                                    #folosit pentru îmbunătățirea preciziei

camera_mono_stanga = pipeline.createMonoCamera()                                        #se adaugă camera mono din partea stângă
camera_mono_dreapta = pipeline.createMonoCamera()                                       #se adaugă camera mono din partea dreapta
camera_mono_stanga.setResolution(dai.MonoCameraProperties.SensorResolution.THE_400_P)   #setează rezoluția camerei din partea stângă, are o precizie medie și o viteză de procesare mare
camera_mono_dreapta.setResolution(dai.MonoCameraProperties.SensorResolution.THE_400_P)  #setează rezoluția camerei din partea dreaptă la 640x400
camera_mono_stanga.setBoardSocket(dai.CameraBoardSocket.CAM_B)                          #se leagă la portul fizic al camerei, CAM_B = camera mono din stânga
camera_mono_dreapta.setBoardSocket(dai.CameraBoardSocket.CAM_C)                         #se leagă la portul fizic al camerei, CAM_C = camera mono din dreapta

camera_mono_stanga.out.link(camera_stereo.left)                                         #se conectează ieșirea camerei mono din stânga la intrarea stângă a camerei stereo
camera_mono_dreapta.out.link(camera_stereo.right)                                       #se conectează ieșirea camerei mono din dreapta la intrarea dreaptă a camerei stereo
...
camera_RGB.preview.link(out_RGB.input)                                                  #se leagă la intrarea fluxului de date, camera RGB, definită anterior

out_adancime = pipeline.createXLinkOut()                                                #se creează un alt nod ce va fi folosit pentru adâncime
out_adancime.setStreamName("adancime")                                                  #se seteaza numele fluxului de date ce va fi responsabil de adâncime
camera_stereo.depth.link(out_adancime.input)                                            #se leagă la intrarea fluxului de date, camera stereo (folosită pt adâncime), definită anterior

with dai.Device(pipeline) as device:                                                    #creează conexiunea între program și camera OAK-D Lite printr-un pipeline
                                                                                        #se pornește camera
    coada_RGB = device.getOutputQueue(name="RGB", maxSize=4, blocking=False)            #creează o coadă, de 4 cadre, în care se stochează cadrele RGB
    coada_adancime = device.getOutputQueue(name="adancime", maxSize=4, blocking=False)  #creează o coadă, de 4 cadre, în care se stochează harta de adâncime
    ...
    cv2.namedWindow("Imagine camera")                                                   #se dă nume ferestrei ce urmează să fie afișată pe ecran
    cv2.setMouseCallback("Imagine camera", desenare_chenar_culoare)                     #leagă funcția ce se ocupă de mișcările mouse-ului, la fereastra dată
    ...
    while True:
        ...
        clona = cadru_BGR.copy()                                                        #se face o copie a imaginii originale, pentru a lucra pe o copie, nu pe original
        
        if not obiect_selectat and conturare_obiect:                                    #dacă nu avem un obiect selectat, dar suntem în procesul de selectare
            cv2.rectangle(clona, (colt_stanga_sus_X, colt_stanga_sus_Y), (colt_stanga_sus_X + lung_chenar, colt_stanga_sus_Y + latime_chenar), (0, 255, 0), 2)
            #se desenează, pe imaginea clona, chenarul de culoare verde, de grosime 2, ce este desenat în timp real de utilizator, pentru a ce zonă selectează
        
        if limita_inferioara_culoare is not None and limita_superioara_culoare is not None:         #dacă există selectată o zonă din imagine, 
                                                                                                    #se formează automat intervalul de culoare în care se caută
            img_alb_negru = cv2.inRange(cadru_HSV, limita_inferioara_culoare, limita_superioara_culoare)     #se creează o imagine alb-negru, unde pixeli ce se află în acel interval
            ...
            if contururi:                                                               #daca există lista de contururi
                ...
                if cv2.contourArea(contur) > 100:                                       #dacă conturul este suficient de mare, eu l-am luat 100 de pixeli
                    ...
                    cv2.circle(clona, (x, y), rază, (0, 255, 0), 2)                     #se desenează conturul cercului găsit, cu culoarea verde și o grosime de 2 pixeli
                    cv2.circle(clona, (x, y), 2, (0, 0, 255), -1)                       #se pune un punct în centrul cercului
                    ...
                    pct_adancime_X = int(x * matrice_adancime.shape[1] / cadru_BGR.shape[1])    #coordonata X a punctului corespondent în imaginea de adâncime; .shape[1]- lățimea imaginii
                    pct_adancime_Y = int(y * matrice_adancime.shape[0] / cadru_BGR.shape[0])    #coordonata Y a punctului corespondent în imaginea de adâncime; .shape[0]- lungimea imaginii
                    
                    pct_adancime_X = max(0, min(pct_adancime_X, matrice_adancime.shape[1] - 1)) #verificăm noile coordonate pentru a nu ieși din imaginea de adâncime
                    pct_adancime_Y = max(0, min(pct_adancime_Y, matrice_adancime.shape[0] - 1))
                
                    zona = 5                                                            #lungimea pătratului care este folosit pentru a analiza adâncimea
                                                                                        #nu ne uitâm doar în centrul cercului, ci pe o zona de 5x5 pixeli, pentru a oferi o stabiliate mai mare
                    #se definesc colțurile pătratului ce se formează în jurul centrului cercului
                    xMin = max(0, pct_adancime_X - zona)                                #ne asigurăm că nu mergem prea în stânga
                    xMax = min(matrice_adancime.shape[1] - 1, pct_adancime_X + zona)    #ne asigurăm că nu mergem prea în dreapta
                    yMin = max(0, pct_adancime_Y - zona)                                #ne asigurăm că nu mergem prea sus
                    ...
                    if len(regiune_adancime_corect) > 0:                                #dacă există astfel de date
                        adancime_out = np.median(regiune_adancime_corect)               #calculează mediana din acea regiune și NU media valorilor
                                                                                        #mediana = valoarea din mijloc care oferă o estimare mai sigură
                        
                        Z = adancime_out / 10.0                                         #convertește valorea distanței în centimetri
                        ...
                        scrie_distanta_in_fisier(X, Y)                                                      #coordonatele punctului sunt transmise ca și parametru pt a se calcula distanța până la inele
